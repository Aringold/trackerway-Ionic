(function () {
  angular.module("rewire.trackerConfig").controller("addObjectController", addObjectController);

  addObjectController.$inject = ["$scope", "toastr", "$http", "global", "utilService", "homeService", "$ionicHistory"];

  function addObjectController($scope, toastr, $http, global, utilService, homeService, $ionicHistory) {
    var vm = this;
    vm.working = false;
    vm.objectTypes = [];
    ...
    function getObjectTypes() {
      ...
      vm.formData.device = "Rewire Security 102-NANO";
    }
    ...
    function save() {
      ...
      utilService.getItem("user", function (user) {
        ...
        vm.formData.user_id = user.id;
        $http.post(global.baseUrl + "/api/mobilapp/addObject.php", vm.formData).then(function (response) {
          vm.working = false;
          if (response.data == "EXISTS") {
            toastr.warning("This IMEI already exists.");
          } else if (response.data = "OK") {
            homeService.getUserObjects(function () {
              toastr.success("Object saved successfuly.");
              $ionicHistory.goBack();
            });
          }
        });
...
gsValues['device_list'] = [
  {name: 'Rewire Security 102-NANO'},
  {name: 'Rewire Security 104-PRO'},
  {name: 'Rewire Security 103-RS'},
  {name: 'Rewire Security 303-FLEET'},
  {name: 'Rewire Security DB1'},
  {name: 'Rewire Security DB1-Lite'},
  {name: 'Rewire Security DB3'},
  {name: 'Rewire Security G1 WIRELESS'},
